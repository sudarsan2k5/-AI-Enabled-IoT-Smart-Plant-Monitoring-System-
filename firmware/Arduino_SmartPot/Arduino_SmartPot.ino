...
void ntp_start() {
  ...
  Serial.println("NTP: failed — 1970 timestamp until NTP works");
}
...
bool read_npk(int &n, int &p, int &k) {
  ...
  if (r != 0) {
    if (r == ModbusMaster::ku8MBResponseTimedOut) {
      Serial.println("NPK: 0xE2=TIMEOUT. Try: swap A<->B, baud, slave ID, registers (see manual).");
    } else
...
bool post_sensor_json(float temp, float hum, int soilPct, int lightVal, int n,
  ...
  if (code < 0)
    Serial.println("POST -1: no TCP connection. PC IP+port, uvicorn 0.0.0.0, firewall.");
  else
